...
auto game_level_data_loader::CreateType2Enemies(level_container* levelContainer, int count) -> void
{
  for( int i = 0; i < count; ++i )
  {
    // levelContainer->CreateEnemyType2(POINT_2F { 0, 0 }, 3, 100.0f, 2.0f, ValidCells());
    const auto& cells = ValidCells();
    cells;
    levelContainer->CreateEnemyType2(POINT_2F { 0, 0 }, 3, 100.0f, 2.0f, cells);
  }
}
